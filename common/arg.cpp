...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--remeasure"},
        "discard cached device performance measurements and measure again",
        [](common_params & params) {
            params.fit_advisor_remeasure = true;
        }
    ).set_examples({LLAMA_EXAMPLE_FIT_ADVISOR}));
    add_opt(common_arg(
        {"--no-measure"},
        "skip device performance measurements, only project memory",
        [](common_params & params) {
            params.fit_advisor_no_measure = true;
        }
    ).set_examples({LLAMA_EXAMPLE_FIT_ADVISOR}));
    add_opt(common_arg(
        {"--measure-types"}, "TYPE,...",
        "extra weight types to measure in addition to the model's own, e.g. q4_K,iq4_xs",
        [](common_params & params, const std::string & value) {
            params.fit_advisor_measure_types = value;
        }
    ).set_examples({LLAMA_EXAMPLE_FIT_ADVISOR}));
    add_opt(common_arg(
        {"--verify"},
    ...
    add_opt(common_arg(
        {"--search-iters"}, "N",
        "annealing iterations for the allocation search (default: 20000, 0 = exact knapsack seeds only)",
        [](common_params & params, int value) {
            params.fit_advisor_search_iters = value;
        }
    ).set_examples({LLAMA_EXAMPLE_FIT_ADVISOR}));
    add_opt(common_arg(
        {"--emit-ini"}, "PATH",
        "write the chosen allocation as a preset section to this INI file (created or updated in place)",
        [](common_params & params, const std::string & value) {
            params.fit_advisor_emit_ini = value;
        }
    ).set_examples({LLAMA_EXAMPLE_FIT_ADVISOR}));
    add_opt(common_arg(
        {"--validate"},
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            params.fit_advisor_emit_name = value;
        }
    ).set_examples({LLAMA_EXAMPLE_FIT_ADVISOR}));
    add_opt(common_arg(
